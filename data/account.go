...
func (t *Account) GetAll(page *int, size *int, condition *up.AndExpr) ([]*Account, int, error) {
	...
	total, err := res.Count()
	if err != nil {
		return nil, -1, newErrors.Wrap(err, "upper count")
	}
	...
	if err != nil {
		return nil, -2, newErrors.Wrap(err, "order accounts")
	}
...
func (t *Account) Update(ctx context.Context, m Account) error {
	...
	if !ok {
		return newErrors.Wrap(nil, "context get")
	}
...
func (t *Account) Delete(ctx context.Context, id int) error {
	...
	userID, ok := contextutil.GetUserIDFromContext(ctx)
	if !ok {
		return newErrors.Wrap(nil, "context get user id")
	}
	...
	err := Upper.Tx(func(sess up.Session) error {
		...
		if err := res.Delete(); err != nil {
			return newErrors.Wrap(err, "upper delete account")
		}
...
func (t *Account) Insert(ctx context.Context, m Account) (int, error) {
	...
	userID, ok := contextutil.GetUserIDFromContext(ctx)
	if !ok {
		return 0, newErrors.Wrap(nil, "context get")
	}
